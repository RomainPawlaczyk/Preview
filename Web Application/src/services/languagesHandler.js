...
export function getLoginText(position, language) {
  ...
  switch (position) {
    ...
    case 17:
      ...
      break;
    case 18:
      language === 'english' ? ret = 'Unexpected server error, please try later' : ret = 'Une erreur est survenue sur le serveur. Merci de réésayer plus tard';
      break;
    case 19:
...
//handle language display for the language selection page
export function getLanguageSelectionText(position, language) {
  let ret = '';
  switch (position) {
    case 1:
      language === 'english' ? ret = 'Please select application language' : ret = "Selectionnez la langue utilisée par l'application";
      break;
    case 2:
...
export function getRegisterText(position, language) {
  ...
  switch (position) {
    ...
    case 2:
      ...
      break;
    case 3:
      language === 'english' ? ret = 'An error occured, email can be already used' : ret = "Une erreur est survenue, l'adresse email peut être déjà utilisée";
      break;
    case 4:
      language === 'english' ? ret = 'All fields must be filled' : ret = 'Merci de remplir tout les champs';
      break;
